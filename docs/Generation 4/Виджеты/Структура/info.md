...
Привязка к типу виджетлиста. Разделы страницы:
- Верхняя панель
- Шапка
- Контент
- Подвал
...
- system-banner-mansonry | Поля: Название (Текст),Контент (HTML), Ссылка (Текст), Изображение (Файл), Соотношение сторон (В / Ш) (Выпадающий список)
- <a name="blocks_example"></a>system-banner-video | Поля: Ссылка (Текст) ,Заставка (Файл)
...
```
{
  ...
  "name": {
    "ru": "Комментарии к статье",
    "es": "Comentarios sobre el artículo - 1"
  },
  "description": {
    "ru": "Список комментариев к статье.",
    "es": "Lista de comentarios al artículo."
  },
  "handle":"benefits_1",
  "page_kinds":["all"],
...
- commonjs_v2 | Фреймворк insales, документация: https://liquidhub.ru/collection/start
- jquery | Документация: https://jquery.com/
- microalert	| Github: https://github.com/VladimirIvanin/microAlert
