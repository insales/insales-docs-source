...
Комментарий к позиции заказа. Для того чтобы поле работало необходимо, чтобы комментарии к заказам были включены в админ-панели, в разделе "Настройки" > "Оформление заказа"
